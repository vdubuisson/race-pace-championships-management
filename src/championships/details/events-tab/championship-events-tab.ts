...
export default class ChampionshipEventsTab {
  ...
  readonly mandatoryEventsCount = computed(() => this.events().filter(event => event.mandatory).length);
  readonly optionalEventsCount = computed(() => this.events().filter(event => !event.mandatory).length);
}
